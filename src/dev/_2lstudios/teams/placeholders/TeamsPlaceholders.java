...
public class TeamsPlaceholders extends PlaceholderExpansion {
  ...
  private boolean isArcherTagged(String name) {
    ...
    if (server.getPluginManager().isPluginEnabled("Classes")) {
      Player player = server.getPlayer(name);
      if (player != null) {
        ClassPlayer classPlayer = Classes.getClassPlayerManager().get(player);
        return classPlayer.hasArcherTag();
      } 
    } 
    return false;
  }
              ...
              if (targetTeamName != null) {
                ...
                if (team != null) {
                  ...
                  if (team1 != null) {
                    ...
                    if (relation == Relation.ENEMY && isArcherTagged(tPlayer1.getName()))
                      return "&e"; 
